...
axf:
	@elf_file=$$(find ./binaries/stm32_stm3210c -type f -name '*.elf' -print -quit); \
	if [ -n "$$elf_file" ]; then \
		mv "$$elf_file" ./binaries/stm32_stm3210c/LearnInDepth_lab.elf; \
		cp ./binaries/stm32_stm3210c/LearnInDepth_lab.elf ./binaries/stm32_stm3210c/LearnInDepth_lab.axf; \
		echo "  >>>>>>>>> LearnInDepth_lab.elf & .axf   DONE  <<<<<<<<<"; \
	else \
		echo "Error: No .elf file found in ./binaries/stm32_stm3210c"; \
	fi
	@echo "  >>>>>>>>> you did great job : professional embedded system engineer ;) <<<<<<<<<"
	@echo "  >>>>>>>>> God grant you success and bless your life :) <<<<<<<<<"
